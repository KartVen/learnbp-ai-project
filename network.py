...
class Network(object):
    ...
    def train(self, training_data, epochs, eta, test_data=None):
        for i_epoch in range(epochs):
            ...
            for i_record in range(training_data.__len__()):
                ...
                outputs = [training_data[i_record][0]]
                ...
                # Backward pass
                errorOutputLayer = self.__cost_derivative(outputs[outputs.__len__() - 1], training_data[i_record][1]) * self.__d_sigmoid(weightedSums[weightedSums.__len__() - 1])
            ...
            for i_layer in range(self.weights.__len__()):
                self.weights[i_layer] = self.weights[i_layer] - weightsGradient[i_layer] * (eta / len(training_data))

            # Update biases
            for i_layer in range(self.biases.__len__()):
                self.biases[i_layer] = self.biases[i_layer] - biasesGradient[i_layer] * (eta / len(training_data))
